...
class TestDisco:
    ...
    def test_disco(self):
        with Disco(broker=self.broker, read_topic=self.read_topic, write_topic=self.write_topic) as self.disco:
...
#        cls.distributedlock.run()
        ...
        cls.lock_process = mp.Process(target=runlock, args=(mynode, peerlist, leader_state))
